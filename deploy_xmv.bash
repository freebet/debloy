...
cd /usr/local/src
sudo git clone https://github.com/loki-project/loki.git
cd loki
sudo make -j$(nproc)
...
sudo systemctl enable monerov
sudo systemctl start monerov
...
mysql -u root --password=$ROOT_SQL_PASS < deployment/basexmv.sql
